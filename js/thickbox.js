...
function tb_init(domChunk) {
  $(domChunk).click(function () {
    ...
    currentImage = parseInt(this.id) + 1;

    galleryImages = [].map.call(
      document.getElementsByClassName("photo-gallery-image"),
      function (currentValue) {
        return {src: currentValue.href || "", caption: currentValue.title || ""};
      }
    );
...
function tb_show(caption, url, imageGroup) {
  ...
  try {
    ...
    if (
      ...
      var totalGalleryImages = galleryImages.length;
      if (totalGalleryImages > 1) {
        if (currentImage > 1) {
          TB_PrevHTML =
            "<span id='PG_prev' style='display: block;'>&nbsp;&nbsp;<a href='#' onclick='pg_prev()'>&lt; Prev</a></span>";
        } else {
          TB_PrevHTML =
            "<span id='PG_prev' style='display: none;'>&nbsp;&nbsp;<a href='#' onclick='pg_prev()'>&lt; Prev</a></span>";
        }
        if (currentImage < totalGalleryImages) {
          TB_NextHTML =
            "<span id='PG_next' style='display: block;'>&nbsp;&nbsp;<a href='#' onclick='pg_next()'>Next &gt;</a></span>";
        } else {
          TB_NextHTML =
            "<span id='PG_next' style='display: none;'>&nbsp;&nbsp;<a href='#' onclick='pg_next()'>Next &gt;</a></span>";
        }
      }
      ...
      imgPreloader.onload = function () {
        ...
        $("#TB_window").append(
          "<a href='' id='TB_ImageOff' title='Close'><img id='TB_Image' src='" +
            ...
            "<div id='TB_caption'>" +
            caption +
            "<div id='TB_secondLine' style='display: flex;'>" +
            TB_imageCount +
            "</div></div><div id='TB_closeWindow'><p class='lightbox-link-3'><a onclick='tb_remove()'></a></p><div><a href='#' id='TB_closeWindowButton' title='Close'>close</a> or Esc Key</div><div class='prev-next-btn'>"+TB_PrevHTML+ " " +TB_NextHTML+ "</div></div>"
        );
...
function pg_prev() {
  currentImage--;
  var currImg = galleryImages[currentImage - 1]['src'];
  var currCaption = galleryImages[currentImage - 1]['caption'];

  $("#PG_prev").css({
    display: currentImage > 1 ? "block" : "none",
  });
  $("#PG_next").css({
    display: currentImage + 1 > galleryImages.length ? "none" : "block",
  });

  imgPreloader1 = new Image();
  imgPreloader1.src = currImg;
  imgPreloader1.onload = function () {
    imgPreloader1.onload = null;

    // Resizing large images - orginal by Christian Montoya edited by me.
    var pagesize = tb_getPageSize();
    var x = pagesize[0] - 150;
    var y = pagesize[1] - 150;
    var imageWidth = imgPreloader1.width;
    var imageHeight = imgPreloader1.height;
    if (imageWidth > x) {
      imageHeight = imageHeight * (x / imageWidth);
      imageWidth = x;
      if (imageHeight > y) {
    ...
    $("#TB_caption").attr("innerText", currCaption);
    $("#TB_Image").attr("src", currImg);
    $("#TB_Image").attr("height", imageHeight);
    $("#TB_Image").attr("width", imageWidth);
    tb_position();
  }
}
