...
function update_31_10_14_getEventData(start,end) 
{
  ...
  //console.log("getting refreshed data...");
   ...
   var jsonParams = '{"calId":"' + php_calId + '", "commId":"' + php_commId + '", "sunday":"' + firstSunday + '","serviceId":"' + serviceId + '"}'; //create the params in json format
  //console.log('params:' + jsonParams);
...
/*EVERYTHING BELOW HERE IS TO RENDER THE CALENDAR*/
      ...
      eventNew : function(calEvent, $event) {
        ...
        var cantBookNow_bookingDays = 0;
         ...
         var bookingDaysReal = 10;
            ...
            if(dateDiffToday > bookingDays){
             alert('You can book this starting '+bookingDaysReal+' days in advance');
                $('#calendar_update_31_10_14').weekCalendar("removeUnsavedEvents");
                return;
        	}
         }
         ...
         update_31_10_14_resetForm($dialogContent);
		  var hour, minutes;
		  minutes = calEvent.start.getMinutes();
		  if(minutes == "0" ){
			  minutes= "00";
		  }
		  hour = calEvent.start.getHours();
		   var beginOnArr;
	   	  if(beginOn){
		  beginOnArr  = beginOn.split(',');
	   		}
		  console.log(beginOnArr);
		  if($.inArray(""+minutes, beginOnArr ) !== -1){
			//  console.log((beginOnArr.indexOf(minutes) > -1));
			  
		  }else{
			  
			   console.log("New Event start_time: "+ hour +" "+ minutes);
			  
			 switch(minutes) {
			  case 00:
					 calEvent.start.setMinutes(15);
				// code block
				break;
			  case 15:
				//alert("minutes");
				calEvent.start.setMinutes(0);
				// code block
				break;
			  case 30:
				calEvent.start.setHours(hour+1);
				calEvent.start.setMinutes(0);
				// code block
				break;
			  case 45:
			   ...
			   if($.inArray("00", beginOnArr )> -1){
					calEvent.start.setMinutes(00);
					
				}
				if($.inArray("15", beginOnArr )> -1){
					calEvent.start.setMinutes(15);
         ...
         var noteField = $dialogContent.find("textarea[name='note']");
      ...
      eventClick : function(calEvent, $event) {
		...
		if(isOwner_update_31_10_14)
		{
         ...
         $dialogContent.dialog({
		  ...
		  console.log("start time "+ calEvent.start);
          var startField = $dialogContent.find("select[name='start_time']").val(calEvent.start);
          var endField = $dialogContent.find("select[name='end_time']").val(calEvent.end);
          $dialogContent.find('.booking_date_span').text(niceDate)
          $dialogContent.find(".date_holder").text($calendar.weekCalendar("formatDate", calEvent.start));
   ...
   function update_31_10_14_setupStartAndEndTimeFields(defaultStart, defaultEnd, actType) 
   {
	   ...
	   //lets fetch our service details 
	    var serviceParams = '{"commId":"' + php_commId + '", "serviceId":"'+ $("#activity").val()+'"}';
        var service_encoded_json = encodeURIComponent(serviceParams);
	   
        var serviceUrl = '/private/admin/api/v1/booking/serviceDetails?json=' + service_encoded_json + '&key=' + php_privateKey;
        ...
        $.getJSON(serviceUrl, function (data)
        {
            if (data.status == "400") {
              //  alert("Sorry, the chosen service is not available at the time selected");
				...
				beginOn = data.response.serviceData.beginOn
            }
        }); 
      ...
      { //loop through each timeslot and figure out the seltector options for the start and end fields
          ...
          if (startTime.getTime() === defaultStart.getTime()) 
          {
     	  ...
     	  if (actType != "na") {
		  	...
		  	slotTime = validTimeslotTimes[i].startFormatted.split(" ");
			slotTime = slotTime[0].split(":");
		  //filter-out-beginOn
			if(jQuery.inArray(slotTime[1], beginArray ) !== -1){
...
//we need to track which activity is being selected 
